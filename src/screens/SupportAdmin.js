          ...
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalOverlay}>
            <Animated.View entering={FadeInDown} style={styles.modalContent}>
              ...
              <View style={styles.modalHeader}>
                ...
                <Text style={styles.modalTitle}>ملف الطالب</Text>
              </View>
              ...
              {selectedStudent && (
                <ScrollView showsVerticalScrollIndicator={false}>
                  ...
                  <View style={styles.passwordCard}>
                      <Text style={styles.cardLabel}>إدارة كلمة المرور</Text>
                      <View style={styles.currentPassBox}>
                          <Text style={styles.passValue}>{selectedStudent.password || "----"}</Text>
                          <Text style={styles.passLabel}>الحالية</Text>
                      </View>
                      <TextInput 
                      ...
                      />
                      <TouchableOpacity style={styles.updateBtn} onPress={async () => {
                        if(newPassword) {
                           await updateDoc(doc(db, "students", selectedStudent.id), { password: newPassword });
                           Alert.alert("نجاح", "تم التحديث");
                           setNewPassword('');
                        }
                      }}>
...
function createStyles(COLORS) {
  return StyleSheet.create({
    ...
    statLabel: { color: COLORS.textSub, fontSize: 12, fontWeight: '700' },
    ...
    searchInput: { flex: 1, color: COLORS.text, textAlign: 'right', fontSize: 16, fontWeight: '700' },
    ...
    studentName: { color: COLORS.text, fontSize: 18, fontWeight: '900' },
    ...
    cardInfo: { color: COLORS.textSub, fontSize: 12 },
    arrow: { position: 'absolute', left: 15, top: '40%' },
    emptyText: { color: COLORS.textSub, textAlign: 'center', marginTop: 40 },
    ...
    avatarText: { color: '#000', fontSize: 30, fontWeight: 'bold' },
    infoName: { color: COLORS.text, fontSize: 20, fontWeight: 'bold' },
    infoUser: { color: COLORS.gold, fontSize: 15, fontWeight: '800' },
    ...
    cardLabel: { color: COLORS.text, fontWeight: 'bold', marginBottom: 15, textAlign: 'right' },
    ...
    passLabel: { color: COLORS.textSub, fontSize: 12 },
    modalInput: { backgroundColor: COLORS.bg, borderRadius: 15, padding: 15, color: COLORS.text, textAlign: 'right', marginBottom: 12, borderWidth: 1, borderColor: COLORS.border },
